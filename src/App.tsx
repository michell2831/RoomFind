...
import { Room, User, Schedule, DashboardStats, AccessLog, RFIDCard, Device, RoomSession } from "./types";
...
function App() {
  ...
  const fetchDevices = async () => {
    try {
      ...
      if (data) {
        ...
        setDevices(formattedDevices as Device[]);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const unsubscribeGranted = iotService.subscribe('access_granted', async (event: IoTEvent) => {
      ...
      if (data.log) {
        setLogs(prev => [...prev, { ...data.log, id: Date.now() }]);
      }
      // Re-fetch rooms to get the latest status set by the backend
      fetchRooms();
      fetchRoomSessions();
    });
  ...
  const addSchedule = async (s: Omit<Schedule, "id" | "created_at" | "room_code" | "room_name" | "faculty_name">) => {
    if (s.start_time >= s.end_time) {
      throw new Error('Invalid schedule time: start time must be earlier than end time.');
    }
  ...
  const updateSchedule = async (id: number, updates: Partial<Schedule>) => {
    ...
    if (proposedStart && proposedEnd && proposedStart >= proposedEnd) {
      alert('Invalid schedule time: start time must be earlier than end time.');
      return;
    }
