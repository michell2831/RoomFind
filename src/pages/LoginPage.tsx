...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  React.useEffect(() => {
    const fetchSections = async () => {
      try {
        const { data, error: secError } = await supabase.from("sections").select("id, name").order("name");
        if (!secError && data) {
          setSections(data);
          if (data.length > 0 && !courseAndSection) {
            setCourseAndSection(data[0].name);
          }
        }
      } catch (err) {
    ...
    try {
      if (isSignUp) {
        if (!email.toLowerCase().endsWith("@iskolarngbayan.pup.edu.ph")) {
          setError("Registration is only allowed using a PUP school email address.");
          setLoading(false);
          return;
        }
        ...
        try {
          ...
          const { data: authData, error: authError } = await supabase.auth.signUp({
            ...
            options: {
              data: {
                name: name,
                department: courseAndSection || 'General'
              }
            }
          });
          ...
          {isForgotPassword ? (
            ...
            <div className="flex flex-col items-center justify-center text-center py-4 space-y-4 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[#94A3B8] text-sm leading-relaxed">
                  A verification link has been sent to <span className="text-white font-semibold">{email}</span>. Please check your email to verify your account before logging in.
                </p>
              </div>
              <button
                type="button"
                      ...
                      <div className="relative">
                        ...
                        {sections.length > 0 ? (
                          <select
                            ...
                            required={isSignUp}
                            className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-8 py-2.5 text-white text-sm focus:outline-none focus:border-[#00D4AA]/50 focus:ring-1 focus:ring-[#00D4AA]/30 transition-all appearance-none"
                          >
