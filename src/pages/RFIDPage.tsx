          ...
          <div className="bg-[#0F1729] rounded-2xl border border-white/10 w-full max-w-sm shadow-2xl">
            ...
            <form onSubmit={handleAssign} className="p-6 space-y-4">
              <div>
                ...
                >
                  <option value="" disabled style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>Select a user...</option>
                  {usersWithoutCards.map(u => (
                    <option key={u.id} value={u.id} style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>{u.name} ({u.email})</option>
                  ))}
                </select>
              </div>
              <div>
