...
export default function SchedulesPage({
  ...
  const visibleSchedules = isAdmin ? schedules : schedules.filter((s) => (s.faculty_id ?? s.user_id) === currentUserId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSchedule) {
        ...
        const scheduleDayText = form.date ? `${finalForm.day_of_week} (${form.date})` : form.day_of_week;
        setOutcomeModal({
          type: "success",
          title: "Schedule Created Successfully!",
          message: `Successfully booked ${form.subject} (${form.section}) inside room ${room ? room.room_code : 'Unknown'} for ${scheduleDayText} at ${form.start_time} - ${form.end_time}.`,
        });
      }
      setShowModal(false);
    } catch (err: any) {
        ...
        <div className="bg-[#0F1729] rounded-xl border border-white/5 p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {rooms.map((room) => {
              ...
              const timedOutSession = roomSessions.find((s) => s.room_id === room.id && s.status === "timed_out");
              const session = activeSession || timedOutSession || null;
              ...
              const statusStyles = activeSession
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <option value={0} style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>Select room...</option>
                    {rooms.map((r) => (
                      <option key={r.id} value={r.id} style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>{r.room_code} – {r.name}</option>
                    ))}
                  </select>
                </div>
                {isAdmin && (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#00D4AA]/50"
                    >
                      <option value={0} style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>Select faculty...</option>
                      {users.filter((u) => u.role === "faculty").map((u) => (
                        <option key={u.id} value={u.id} style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>{u.name}</option>
                      ))}
                    </select>
                  </div>
                )}
              ...
              <div>
                ...
                >
                  {DAYS.map((d) => <option key={d} value={d} style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>{d}</option>)}
                </select>
                {form.date && (
