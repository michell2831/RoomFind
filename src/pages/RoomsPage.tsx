...
export default function RoomsPage({ rooms, devices = [], schedules = [], onAddRoom, onUpdateRoom, onDeleteRoom, isAdmin }: RoomsPageProps) {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
              {["Room Code", "Name", "Building", "Floor", "Capacity", "Status", "Connected IoT", "Schedules", "Features", isAdmin ? "Actions" : ""].filter(Boolean).map((h) => (
            ...
            {filtered.map((room, idx) => {
              ...
              return (
                <React.Fragment key={room.id}>
                        ...
                        {roomDevices.length > 0 ? (
                          roomDevices.map((device) => (
                            <div
                              key={device.id}
                              className={cn(
                                "flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] border font-mono transition-all",
                                device.status === 'online'
                                  ? "bg-[#00D4AA]/5 text-[#00D4AA] border-[#00D4AA]/20"
                                  : "bg-red-500/5 text-red-400 border-red-500/10"
                              )}
                              title={`${device.device_uid} (${device.status})`}
                            >
                              <div className={cn("w-1.5 h-1.5 rounded-full", device.status === 'online' ? "bg-[#00D4AA] animate-pulse" : "bg-red-400")} />
                              <span>{device.device_uid.slice(-6)}</span>
                            </div>
                          ))
                        ) : (
                          <span className="text-[10px] text-[#94A3B8]/40 italic">None</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[#94A3B8] text-xs">
                      {roomSchedules.length === 0 ? (
                        ...
                        <button
                          ...
                          className="text-[#00D4AA] hover:text-[#00D4AA]/90 text-xs font-semibold"
                        >
                          {isExpanded ? "Hide schedules" : `View schedules (${roomSchedules.length})`}
                        </button>
                      )}
                    </td>
                    <td className="px-4 py-3">
                  ...
                  {isExpanded && roomSchedules.length > 0 && (
                    <tr className="border-b border-white/5">
                      <td colSpan={isAdmin ? 10 : 9} className="px-4 py-3 bg-white/2">
                        <div className="rounded-lg border border-white/5 overflow-hidden">
                          <table className="w-full">
                            <thead>
                              <tr className="bg-white/3">
                                {["Day", "Time", "Faculty", "Subject", "Section"].map((h) => (
                                ...
                                <tr key={s.id} className="border-t border-white/5">
                                  <td className="px-3 py-2 text-[11px]">
                                    <div className="flex flex-col">
                                      <span className="text-[#94A3B8] font-medium">{s.day_of_week}</span>
                                      {s.date && (
                                  ...
                                  <td className="px-3 py-2 text-[#94A3B8] text-[11px] font-mono">
                                    {String(s.start_time ?? s.time_start ?? '').slice(0, 5)}–{String(s.end_time ?? s.time_end ?? '').slice(0, 5)}
                                  </td>
                                  <td className="px-3 py-2 text-[#94A3B8] text-[11px]">{s.faculty_name}</td>
            ...
            <div key={room.id} className="bg-[#0F1729] rounded-xl border border-white/5 p-4 hover:shadow-lg transition">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-3 text-[#94A3B8] text-xs flex flex-col gap-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[#94A3B8] text-xs">IoT:</span>
                      <div className="flex flex-wrap gap-1">
                        {roomDevices.length > 0 ? (
                          roomDevices.map((device) => (
                            <span
                              key={device.id}
                              className={cn(
                                "inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] border font-mono",
                                device.status === 'online'
                                  ? "bg-[#00D4AA]/5 text-[#00D4AA] border-[#00D4AA]/20"
                                  : "bg-red-500/5 text-red-400 border-red-500/10"
                              )}
                            >
                              <span className={cn("w-1 h-1 rounded-full", device.status === 'online' ? "bg-[#00D4AA] animate-pulse" : "bg-red-400")} />
                              {device.device_uid}
                            </span>
                          ))
                        ) : (
                          <span className="text-[10px] text-[#94A3B8]/30 italic">None</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Mobile Schedules Listing */}
                  <div className="mt-3 pt-3 border-t border-white/5">
                    <p className="text-[10px] uppercase text-[#94A3B8]/60 font-semibold tracking-wider mb-2">Room Schedules</p>
                    {roomSchedules.length === 0 ? (
                      <p className="text-[11px] text-[#94A3B8]/40 italic">No schedules</p>
                    ) : (
                      <div className="space-y-2">
                        {roomSchedules.map((s) => (
                          <div key={s.id} className="text-[11px] flex justify-between bg-white/2 rounded px-2.5 py-2">
                            <div>
                              <span className="font-semibold text-white">{s.subject}</span>
                              <span className="text-[#94A3B8] ml-1">({s.section})</span>
                              <p className="text-[#94A3B8] text-[10px] mt-0.5">{s.faculty_name}</p>
                            </div>
                            <div className="text-right font-mono text-[10px] text-[#94A3B8] flex flex-col justify-center">
                              <p className="text-white font-medium">{s.day_of_week}</p>
                              {s.date && <p className="text-[#64748B] text-[9px]">{s.date}</p>}
                              <p>{String(s.start_time ?? s.time_start ?? '').slice(0, 5)}–{String(s.end_time ?? s.time_end ?? '').slice(0, 5)}</p>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="available" style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>Available</option>
                    <option value="occupied" style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>Occupied</option>
                    <option value="maintenance" style={{ color: '#ffffff', backgroundColor: '#0F1729' }}>Maintenance</option>
                  </select>
                </div>
              </div>
              <div>
